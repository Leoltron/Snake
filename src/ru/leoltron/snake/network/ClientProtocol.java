...
public class ClientProtocol implements WindowListener, CurrentDirectionHolder {
    ...
    public ClientProtocol(JPanel parentPanel, String hostname, int port) {
        try {
            try {
            ...
            new Thread(() -> {
                try {
                    ...
                    while (in != null && (line = in.readLine()) != null) {
                        ...
                        int delayMS = Integer.parseInt(match.group(2));
                        int lastPacketReceivedTick = Integer.parseInt(match.group(3));
    ...
    private void sendDirectionUpdatePacket(Direction direction) {
        out.println(game.getTime() + ":" + direction);
    }
